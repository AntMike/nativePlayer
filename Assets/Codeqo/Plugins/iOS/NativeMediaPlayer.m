...
void _prepare(int _id, const char* _listener, bool _playWhenReady)  {
    [[NativeMediaPlayer sharedObject] prepare:(int)_id listenerName:GetStringParam(_listener) playWhenReady:(bool)_playWhenReady];
}

void _prepareMediaItem(int _id, bool _playWhenReady, bool _newPlaylist)  {
    [[NativeMediaPlayer sharedObject] prepareMediaItem:(int)_id playWhenReady:(bool)_playWhenReady newPlaylist:(bool)_newPlaylist];
}
...
void _addMediaItem(int _id) {
    [[NativeMediaPlayer sharedObject] addMediaItem:(int)_id];
}
...
- (void) prepare:(int)_id listenerName:(NSString*)_listener playWhenReady:(bool)_playWhenReady {

    [self frelease];
    
    int count = [[PlayerPrefs sharedObject] getInt:@"NUM_TRACKS"];
    NSMutableArray *_mediaItems = [[NSMutableArray<MediaItem*> alloc] init];
    for (int i = 0; i < count; ++i) {
        [_mediaItems addObject:[[MediaItem alloc] initWithIndex:i]];
    }

    if (listener != _listener) listener = _listener;

    if (currentMediaLocation == STREAMING_ASSET) {
    ...
    } else if (currentMediaLocation == REMOTE_URL) {
...
- (void) prepareMediaItem:(int)_id playWhenReady:(bool)_playWhenReady newPlaylist:(bool)_newPlaylist {
    if ([self isPlaying]) [self stop];
    if (_newPlaylist) {        
        [self prepare:_id listenerName:listener playWhenReady:_playWhenReady];
    } else {
...
- (bool) isLocal {
    if (currentMediaLocation == STREAMING_ASSET && self.localPlayer != nil) {
        return true;
...
- (void) addMediaItem:(int)_id {
    if ([self isLocal]) [self.localPlayer addMediaItem:_id];
    else if ([self isRemote]) [self.remotePlayer addMediaItem:_id];
}
