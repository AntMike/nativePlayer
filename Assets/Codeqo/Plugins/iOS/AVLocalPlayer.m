...
- (void) prepare {
    ...
    [self performSelector:@selector(checkPlaybackStateReady) withObject:nil afterDelay:.3];
...
- (void) checkPlaybackStateReady {
    readyCheck++;
    if (readyCheck > 30) {
        [unityListener setError:@"AVAudioPlayer failed to start"];
        readyCheck = 0;
        return;
    }
...
- (void) addMediaItem:(int)_id {
    [mediaItems insertObject:[[MediaItem alloc] initWithIndex:_id] atIndex:_id];
